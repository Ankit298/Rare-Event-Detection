...
train_metadata = train_metadata.sample(frac=1,random_state = 0)
test_metadata = test_metadata.sample(frac=1,random_state = 0)
...
def ResNet50(input_shape = (64, 64, 1), classes = 2):
    ...
    return model
    return model
#Weighted binary_crossentropy Loss
def weighted_binary_crossentropy( y_true, y_pred, weight=125 ) :
    y_true = K.clip(y_true, K.epsilon(), 1)
    y_pred = K.clip(y_pred, K.epsilon(), 1)
    logloss = -(y_true * K.log(y_pred + np.finfo(float).eps) * weight + (1 - y_true) * K.log(1 - y_pred + np.finfo(float).eps))
